...
def generate_scenario_5(n, p_dim, lambda_param, k_param, tau):
    mean = np.zeros(p_dim) 
    rho = np.exp(1) 
    def covariance(p):
        cov = np.zeros((p, p))
    ...
    g_Z = np.dot(Z, beta) + np.array([x1 < -0.5 for x1 in Z[:,0]] or [x2 < -0.5 for x2 in Z[:,1]], dtype=int) - np.array([x1 >= -0.5 for x1 in Z[:,0]] or [x2 >= -0.5 for x2 in Z[:,1]], dtype=int) + Z[:,2] * Z[:,3]
    Y = ndm.rand(n)
    T = (-np.log(Y)* np.exp(- g_Z))**(1/k_param) / lambda_param
    U_0 = ndm.exponential(1, n) * tau / 4
    U = np.clip(U_0, 0, 5*tau/6)
    V_0 = tau / 3 + U + ndm.exponential(1, n) * tau / 5
    V = np.clip(V_0, 0, tau)
    De1 = (T <= U)
    De2 = (U < T) * (T <= V)
    De3 = (T > V)
