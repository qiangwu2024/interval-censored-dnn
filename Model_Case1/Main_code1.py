...
node_D = np.array([40, 55]) 
lr_D = np.array([3e-4, 5e-4])

node_L = np.array([40, 50])
lr_L = np.array([3e-4, 5e-4])

node_A = np.array([45, 50]) 
lr_A = np.array([3e-4, 5e-4])

B = 200
#%% test data
test_data = generate_case_I(200, corr, Beta, tau)
X_test = test_data['X']
...
plt.ylim(-2,2)
ax1_1.set_title("Case 1, n=1000",fontsize=10)
...
plt.ylim(-2,2)
ax1_2.set_title("Case 1, n=2000",fontsize=10) 
...
plt.ylim(0,1)
ax2_1.set_title("Case 1, n=1000", fontsize=10)
...
plt.ylim(0,1)
ax2_2.set_title("Case 1, n=2000", fontsize=10)
...
for i in range(len(Set_n)):
    ...
    for b in range(B):
        ...
        # train data
        train_data = generate_case_I(n, corr, Beta, tau)
        ...
        Q2_y = Ef_U/(Ef_U-Ef_V)
        Q3_y = Ef_V/(Ef_U-Ef_V)
        Es = f_U*(De1_train*Q1_y-De2_train*Q2_y)*(Z_train-a_b_D[:,0]) + f_V*(De2_train*Q3_y-De3_train)*(Z_train-a_b_D[:,1])
        beta_D.append(Est_D['Beta'])
        Info_D.append(np.mean(Es**2))
        ...
        Q2_y = Ef_U_A/(Ef_U_A-Ef_V_A)
        Q3_y = Ef_V_A/(Ef_U_A-Ef_V_A)
