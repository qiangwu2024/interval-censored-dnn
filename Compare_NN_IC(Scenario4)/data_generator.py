...
def generate_scenario_4(n, p_dim, lambda_param, k_param, tau):
    mean = np.zeros(p_dim)
    rho = np.exp(1)
    def covariance(p):
        cov = np.zeros((p, p))
    ...
    g_Z = np.dot(Z, beta) + Z[:,0]**2 + Z[:,1]**2 + Z[:,2] * Z[:,3]
    Y = ndm.rand(n)
...
Dat = generate_scenario_4(1000, 20, 0.01, 10, 100)
g = Dat['g_Z']
np.max(g), np.min(g), np.mean(g), np.mean(Dat['De1']), np.mean(Dat['De2']), np.mean(Dat['De3']), np.max(Dat['T']), np.min(Dat['T']), np.max(Dat['U']), np.min(Dat['U']), np.max(Dat['V']), np.min(Dat['V']), np.sum(Dat['V'] < 100), np.sum(Dat['U'] < Dat['V'] )
