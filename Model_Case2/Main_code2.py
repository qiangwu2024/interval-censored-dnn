...
node_D = np.array([50, 55]) 
lr_D = np.array([2e-4, 5e-4])

node_L = np.array([50, 50])
lr_L = np.array([2e-4, 2e-4])

node_A = np.array([50, 55]) 
lr_A = np.array([4e-4, 5e-4])

B = 200 
#%% test data
test_data = generate_case_2(200, corr, Beta, tau)
X_test = test_data['X']
g_true = test_data['g_X']
dim_x = X_test.shape[0] 
u_value = np.array(np.linspace(0, tau, 50), dtype="float32") 
Lambda_true = np.log(1 + u_value ** (4/5) / 6)
m = 10
...
plt.ylim(-2,2) 
ax1_1.set_title("Case 2, n=1000",fontsize=10) 
...
plt.ylim(-2,2) 
ax1_2.set_title("Case 2, n=2000",fontsize=10) 
...
plt.ylim(0,2) 
ax2_1.set_title("Case 2, n=1000", fontsize=10) 
...
plt.ylim(0,2) 
ax2_2.set_title("Case 2, n=2000", fontsize=10) 
...
for i in range(len(Set_n)):
    ...
    for b in range(B):
        ...
        # train data
        train_data = generate_case_2(n, corr, Beta, tau)
...
#%% -----------Save all results------------
# ================figures======================
fig1.savefig('fig_g_2_t.jpeg', dpi=400, bbox_inches='tight')
fig2.savefig('fig_Lambda_2_t.jpeg', dpi=400, bbox_inches='tight')
...
result_error.to_csv('result_error_additive.csv')
...
result_re.to_csv('result_re_additive.csv')