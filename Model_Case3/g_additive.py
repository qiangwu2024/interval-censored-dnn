...
def g_A(train_data,X_test,Lambda_U,Lambda_V,m0,nodevec0):
    ...
    def GA(*args):
        b = args[0]
        Ezg = np.exp(Z_train *b[5*(m0+4)+1] + np.dot(B_0, b[0:(m0+4)]) + np.dot(B_1, b[(m0+4):(2*(m0+4))]) + np.dot(B_2, b[(2*(m0+4)):(3*(m0+4))]) + np.dot(B_3, b[(3*(m0+4)):(4*(m0+4))]) + np.dot(B_4, b[(4*(m0+4)):(5*(m0+4))]) + b[5*(m0+4)]*np.ones(X_train.shape[0]))
        ...
        return loss_fun
    param = spo.minimize(GA,np.zeros(5*(m0+4)+2),method='SLSQP')['x']
