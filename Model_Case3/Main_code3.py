...
corr = 0.5
n_layer = 4
Set_node = np.array([45, 50])
n_epoch = 200
Set_lr = np.array([3.5e-4, 5e-4])
Beta = 1

node_D = np.array([50, 65]) 
lr_D = np.array([3e-4, 5e-4])

node_L = np.array([40, 40])
lr_L = np.array([2e-4, 2e-4])

node_A = np.array([40, 45]) 
lr_A = np.array([2e-4, 3e-4])

B = 200
#%% test data
test_data = generate_case_3(200, corr, Beta, tau)
X_test = test_data['X']
g_true = test_data['g_X']
dim_x = X_test.shape[0]
u_value = np.array(np.linspace(0, tau, 50), dtype="float32")
Lambda_true = 2 * np.sqrt(u_value) / 9
m = 10
...
plt.ylim(-2,2)
ax1_1.set_title("Case 3, n=1000",fontsize=10)
...
plt.ylim(-2,2) 
ax1_2.set_title("Case 3, n=2000",fontsize=10)
...
plt.ylim(0,1.5) 
ax2_2.set_title("Case 3, n=2000", fontsize=10)
...
for i in range(len(Set_n)):
    ...
    for b in range(B):
        ...
        # train data
        train_data = generate_case_3(n, corr, Beta, tau)
...
result_error.to_csv('result_error_deep1.csv')
...
result_re.to_csv('result_re_deep1.csv')