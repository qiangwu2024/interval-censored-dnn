...
def generate_scenario_2(n, p_dim, lambda_param, k_param, tau):
    mean = np.zeros(p_dim)
    rho = np.exp(1)
    def covariance(p):
        cov = np.zeros((p, p))
    ...
    g_Z = np.dot(Z, beta) + Z[:,0]**2 + Z[:,1]**2
    Y = ndm.rand(n)
    T = (-np.log(Y)* np.exp(- g_Z))**(1/k_param) / lambda_param
    U_0 = ndm.exponential(1, n) * tau / 5
    U = np.clip(U_0, 0, 5*tau/6)
    V_0 = tau / 5 + U + ndm.exponential(1, n) * tau / 4
    V = np.clip(V_0, 0, tau)
    De1 = (T <= U)
    De2 = (U < T) * (T <= V)
    De3 = (T > V)
