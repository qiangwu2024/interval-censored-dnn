...
import pandas as pd
import time
from data_generator import generate_scenario_2
...
for k in range(len(p_dim_list)):
    ...
    for b in range(B):
        print('---p=', p_dim,'---b=', b)
        #%% Training parameters
        train_data = generate_scenario_2(n, p_dim, lambda_param, k_param, tau)
        ...
        #%% validation data
        validation_data = generate_scenario_2(200, p_dim, lambda_param, k_param, tau)

        #%% test data
        test_data = generate_scenario_2(200, p_dim, lambda_param, k_param, tau)
        Z_test = test_data['Z']   
...
result_mspe.to_csv('result_scenario2.csv')
run_time_D_all